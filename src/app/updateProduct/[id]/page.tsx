...
type ImageSlot = string | File | null;
...
function UpdateProduct() {
  ...
  const [previewImage1, setPreviewImage1] = useState<ImageSlot>(null);
  const [previewImage2, setPreviewImage2] = useState<ImageSlot>(null);
  const [previewImage3, setPreviewImage3] = useState<ImageSlot>(null);
  const [previewImage4, setPreviewImage4] = useState<ImageSlot>(null);
  ...
  useEffect(() => {
    ...
    setProductDetails({
      ...
      category: categories.includes(product.category) ? product.category : "",
      customCategory: categories.includes(product.category)
        ? ""
        : product.category,
    ...
    setPreviewImage1(product.images?.[0] || null);
    setPreviewImage2(product.images?.[1] || null);
    setPreviewImage3(product.images?.[2] || null);
    setPreviewImage4(product.images?.[3] || null);
  }, []);
  ...
  const getImageSrc = (image: ImageSlot): string => {
    if (!image) return "";
    if (typeof image === "string") return image; // Existing Cloudinary URL from DB
    return URL.createObjectURL(image); // Newly chosen local File
  };
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!previewImage1 || !previewImage2 || !previewImage3 || !previewImage4) {
      toast.error("All 4 image slots are required");
      return;
    }
    setLoading(true);
    const formData = new FormData();
    const slots = [previewImage1, previewImage2, previewImage3, previewImage4];
    ...
    try {
      const res = await axios.post("/api/vendor/updateProduct", formData);
      if (!res?.data?.success) {
  ...
  return (
    <div className="flex bg-gradient-to-tr from-slate-950 via-slate-900 to-slate-950 text-slate-100 min-h-screen justify-center items-center py-16 px-4 font-sans overflow-x-hidden">
      ...
      >
        ...
        >
          {loading ? <ClipLoader size={28} /> : "Update Product"}
        </button>
      </form>
    </div>
  );
